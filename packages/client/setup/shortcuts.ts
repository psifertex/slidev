...
export default function setupShortcuts() {
  ...
  // reveal.js' escape closes whatever overlay is open before it touches the
  // overview (`keyboard.js`: `if( closeOverlay() === false ) overview.toggle()`).
  function onEscape() {
  ...
  let shortcuts: ShortcutOptions[] = [
    ...
    // reveal.js binds ESC and O to *toggle* the overview. Stock Slidev binds
    // escape to hide-only, so on a normal slide it does nothing at all.
